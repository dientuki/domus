...
float valorReposo = 2.50; //2.527
float intensidadPico = 0;
float tensionRed = 230.0;
...
float get_corriente()
{
  ...
  { 
    voltajeSensor = analogRead(A0) * (5.0 / 1023.0);//lectura del sensor en volt
    corriente= 0.9 * corriente + 0.1 * ((voltajeSensor - valorReposo)/Sensibilidad); //Ecuación  para obtener la corriente
